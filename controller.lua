...
while true do
  ...
  -- Read input for previously typed command
  ...
  -- Read new command
	  ...
	  -- EVALS command
	  --
	  -- If exp is number <n>, then controller expects <n> expressions and
	  -- then controller evaluates them like the table.
	  -- If exp is not number then controller tries to evaluate table
	  -- with that name.
	  elseif command == "evals" then
		local _, _, exp = string.find(line, "^[a-z]+%s+(.+)$")
		if exp then
		  client:send("EXEC return (" .. exp .. ")\n")
		  local line = client:receive()
